...
Shows status instantly. Color-coded program cards (on track / at risk / off track), each with progress, open risks, and the next milestone.
Summarizes the portfolio in plain language. The "pulse" banner reads every program and writes a one-line health summary — surfacing the nearest milestone and flagging what's off track, so leadership gets the headline before the detail.
Stays out of the way. No login, no setup, no backend to maintain. Add a program in seconds; data persists privately to your session.
